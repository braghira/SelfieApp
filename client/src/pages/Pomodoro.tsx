import { useState } from "react";

export default function Pomodoro() {
  const [timer, setTimer] = useState(0);

  return (
    <div className="container flex justify-start items-center">
      <svg
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
        className="fill-logo w-[500px] h-[500px]"
      >
        <clipPath id="tomato-clip">
          <circle
            cx="-95"
            cy="256"
            r="800px"
            className="animate-tomato"
            /* change the duration based on the timer selected */
            style={{ animationDuration: "5s" }}
          />
        </clipPath>
        <path
          id="tomato-bottom"
          d="M217.6,88.2c17.6,8,34,10,34,10s19.6-2,40-10h1.6c122.4,0,214.4,76,214.4,182
	c0,116.8-112.8,238.4-252,238.4s-252-121.2-252-238c0-106,92.4-182,213.2-182h0.8V88.2z"
        />
        <path
          id="tomato-top"
          d="M252,98.2c0,0,19.6-2,40-10c21.2-8,43.2-30.8,43.2-70.4c-16,1.6-37.2,8.4-54.8,26.8
	c-6-17.6-14.4-30-24-40.4c-11.2,12.4-19.6,24.4-25.2,40.8 M231.6,45h-1.2c-18-17.6-35.2-25.6-53.6-27.2c0,39.6,21.2,62.8,40.8,70.8
	c17.6,8,34,10,34,10"
        />
        <path
          id="tomato-edge"
          d="M256,511.8C109.6,511.8,0,383.8,0,269C0,162.2,91.2,83.8,217.2,82.2c2,0,4,1.6,4,4s-1.6,4-4,4C96,91.8,8,167,8,269
	c0,110.8,106,234.8,248,234.8s248-124,248-234.8c0-102-88.4-177.2-210.4-179.2c-2.4,0-4-2-4-4s2-4,4-4l0,0
	C420,83.4,512,162.2,512,269C512,383.8,402.4,511.8,256,511.8z"
        />
        <path
          id="tomato-leaf1"
          d="M252,100.2C251.6,100.2,251.6,100.2,252,100.2c-0.8,0-18-1.2-36-10.4c-19.6-10-43.2-30.8-43.2-73.2c0-1.2,0.4-2,1.2-2.8
	s2-1.2,3.2-1.2c20,2,38,11.2,56,28.8c1.6,1.6,1.6,4,0,5.6s-4,1.6-5.6,0c-14.8-14.8-30.4-23.6-46.4-26c1.2,28.4,14,49.2,38.4,61.6
	c16.8,8.4,32.4,9.6,32.4,9.6c2.4,0,4,2,3.6,4.4C255.6,98.6,254,100.2,252,100.2z"
        />
        <path
          id="tomato-leaf2"
          d="M224.4,90.2C224,90.2,224,90.2,224.4,90.2c-2.4,0-4-2-4-4c0.4-16.8,2.8-30.4,6.4-42.4c5.6-17.2,14.4-29.2,26.4-42.4
	c1.6-1.6,4.4-1.6,6,0c11.2,12.4,18.8,26,24.4,42.8c0.8,2-0.4,4.4-2.4,5.2c-2,0.8-4.4-0.4-5.2-2.4c-4.4-14-10.8-25.6-19.6-36.4
	c-10,11.2-16.8,21.6-21.6,36c-3.6,11.2-5.6,24.4-6,40C228,88.6,226.4,90.2,224.4,90.2z"
        />
        <path
          id="tomato-leaf3"
          d="M252,100.2c-1.2,0-2.4-0.4-3.2-1.6s-1.2-2.4-0.8-3.6c0.8-2.4,1.2-4.4,2-6.4c0.4-1.2,0.8-2.4,1.2-3.6
	c5.6-15.6,13.6-29.2,23.6-40.4l0.4-0.4c0.4-0.4,0.8-0.8,1.2-1.2c22.8-24.4,48.4-29.2,58.4-30c1.2,0,2.4,0.4,3.2,1.2
	c0.8,0.8,1.2,2,1.2,2.8c0,42.8-24.8,63.6-45.6,73.2C272.8,99.4,252.8,100.2,252,100.2L252,100.2z M281.2,49.4L281.2,49.4
	c-9.6,10.8-17.2,23.6-22.4,38c-0.4,1.2-0.8,2.4-1.2,3.6v0.4c7.2-0.8,20-3.2,32.8-9.2c26-12,39.6-32.8,40.8-61.2
	c-10.8,1.6-30.4,7.6-48.4,26.8C282,48.6,281.6,49,281.2,49.4z"
        />
        <path
          id="tomato-shine1"
          d="M183.2,471c-0.4,0-0.8,0-1.2-0.4c-32.4-11.6-61.6-30.4-87.2-55.6c-20-19.6-36-42-47.6-65.6c-0.8-2,0-4.4,2-5.2
	c2-0.8,4.4,0,5.2,2c11.2,23.2,26.4,44.4,46,63.6c24.8,24.4,53.2,42.4,84.4,53.6c2,0.8,3.2,3.2,2.4,5.2
	C186.4,470.2,184.8,471,183.2,471z"
        />
        <path
          id="tomato-shine2"
          d="M480,273c-2.4,0-4-1.6-4-4c0-33.6-11.2-62.8-33.6-87.2c-1.6-1.6-1.2-4,0.4-5.6c1.6-1.6,4-1.2,5.6,0.4
	c23.6,26,36,56.8,36,92.4C484,271.4,482.4,273,480,273z"
        />
        <path
          id="tomato-shine3"
          d="M418.4,159.8c-0.8,0-1.6-0.4-2.4-0.8c-10.8-7.2-22.4-14-35.2-19.6c-2-0.8-2.8-3.2-2-5.2s3.2-2.8,5.2-2
	c13.2,5.6,25.6,12.4,36.8,20c2,1.2,2.4,3.6,1.2,5.6C420.8,159.4,419.6,159.8,418.4,159.8z"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-bottom"
          fill="#F44336"
        />
        <use clip-path="url(#tomato-clip)" href="#tomato-top" fill="#689F38" />
        <use clip-path="url(#tomato-clip)" href="#tomato-edge" fill="#000000" />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-leaf1"
          fill="#000000"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-leaf2"
          fill="#000000"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-leaf3"
          fill="#000000"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-shine1"
          fill="#000000"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-shine2"
          fill="#000000"
        />
        <use
          clip-path="url(#tomato-clip)"
          href="#tomato-shine3"
          fill="#000000"
        />
      </svg>
    </div>
  );
}
